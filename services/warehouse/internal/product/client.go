...
for i, id := range ids { parts[i] = "id=" + id.String() }
...
if err != nil { return nil, fmt.Errorf("product unreachable: %w", err) }
...
var out struct{ Items []Product `json:"items"` }
...
if err != nil { return nil, err }
defer resp.Body.Close()
if resp.StatusCode >= 300 { return nil, fmt.Errorf("units error %d", resp.StatusCode) }

var out struct{ Items []Unit `json:"items"` }
if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { return nil, err }
return out.Items, nil
}