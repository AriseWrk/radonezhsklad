...
Write-Host "=== Import MS inventory -> inventories (limit=$Limit) ==="
...
while ($true) {
    ...
    $page = MsApi-Get -Path '/entity/inventory' -Query $q
    ...
    foreach ($d in $page.rows) {
        ...
        if ($expected -gt $positions.Count) {
            $positions = MsApi-GetAll -Path "/entity/inventory/$($d.id)/positions" -PageSize 1000
        }
        ...
        foreach ($p in $positions) {
            ...
            $corrSum = 0
            if ($null -ne $p.correctionSum) { $corrSum = [math]::Round(([double]$p.correctionSum) / 100, 2) }
            $irow = @(
                $d.id,
                $p.id,
                (Esc-Text $prodId),
                (Esc-Num  $p.quantity),
                (Esc-Num  $p.calculatedQuantity),
                (Esc-Num  $p.correctionAmount),
                (Esc-Num  $priceR),
                (Esc-Num  $corrSum)
            ) -join $TAB
            $itemLines.Add($irow)
...
$tpl = Read-Utf8File -Path (Join-Path $root 'scripts\sql\upsert_inventory.sql')
