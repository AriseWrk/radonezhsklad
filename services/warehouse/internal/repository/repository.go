...
func (r *Repo) ListStock(ctx context.Context, warehouseID, productID *uuid.UUID) ([]models.StockBalance, error) {
	...
	i := 1
	if warehouseID != nil {
		q += ` AND warehouse_id = $` + itoa(i)
		args = append(args, *warehouseID)
		i++
	}
	if productID != nil {
		q += ` AND product_id = $` + itoa(i)
...
func (r *Repo) ListDocuments(ctx context.Context, f DocumentFilters) ([]models.Document, error) {
	...
	if f.Status != nil {
		q += ` AND d.status = $` + itoa(i)
		args = append(args, *f.Status)
		i++
	}
	if f.WarehouseID != nil {
		q += ` AND d.warehouse_id = $` + itoa(i)
		args = append(args, *f.WarehouseID)
		i++
	}
	if f.SupplierID != nil {
		q += ` AND d.supplier_id = $` + itoa(i)
...
ORDER BY product_id`, warehouseID)
	...
	for rows.Next() {
		var e InventoryRow
		if err := rows.Scan(&e.ProductID, &e.BookQuantity); err != nil {
			return nil, err
		}
		out = append(out, e)
	}
...
WHERE sb.quantity <> 0`
	...
	for rows.Next() {
		var e ExtendedRow
		if err := rows.Scan(&e.ProductID, &e.WarehouseID, &e.Quantity, &e.LastMovement); err != nil {
			return nil, err
		}
		out = append(out, e)
	}
...
ORDER BY w.name`, productID)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&b.WarehouseID, &b.WarehouseName, &b.Quantity); err != nil {
			return nil, err
		}
		out = append(out, b)
	}
