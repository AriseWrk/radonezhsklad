...
## Этап 45b: Сортировка заказов по doc_date + лимит 20000

### Проблема

В списке /internal-orders после 02070 шёл сразу 02053, хотя в МС между ними
ещё 18 заказов. Причина: List сортировал ORDER BY o.created_at DESC,
а created_at у всех импортированных = одна метка (момент импорта) →
PostgreSQL отдавал произвольные 500 строк.

### Фикс

- repository/internal_orders.go:
  ORDER BY o.created_at DESC LIMIT 500
  → ORDER BY o.doc_date DESC NULLS LAST, o.id DESC LIMIT 20000

Второе — в БД 11 091 заказ, а API отдавал только 500 (см. UI: «1-100 из N»,
но фактически пагинация ограничена 500). Подняли лимит до 20000.

### Проверки

- API /internal-orders: 11 091 заказ.
- Топ-20: 02070, 02069, 02068, 02067, 02066, 02065, 02064, 02059, 02063,
  02062, 02061, 02060, 02052, 02058, 02050, 02049, 02056, 02055, 02054, 02053
  — идентично МС.
- Все проверочные номера (00886, 02069, 01987, 00001, 00019, 00049) находятся.
