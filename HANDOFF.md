...
- Внутренние заказы: карточка InternalOrderCardView.vue уже есть, но работает только
  для CRUD, не просматривает импортированные (11090). Расширить под read-only просмотр.
