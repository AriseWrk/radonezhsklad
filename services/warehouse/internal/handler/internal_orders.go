...
func (h *InternalOrderHandler) toInput(c *gin.Context, req intOrderReq) (repository.InternalOrderInput, error) {
	...
	for _, it := range req.Items {
		...
		if err != nil {
			return repository.InternalOrderInput{}, err
		}
		if it.Quantity <= 0 {
			return repository.InternalOrderInput{}, apperr.BadRequest("quantity must be > 0")
		}
		vat := it.VatRate
		if vat == 0 {
			vat = 20
...
func (h *InternalOrderHandler) Post(c *gin.Context) {
	...
	// push в МС (после локального коммита); ошибка уже записана в o.ms_sync_error
	if h.pusher != nil && h.pusher.Enabled() {
		token := strings.TrimPrefix(c.GetHeader("Authorization"), "Bearer ")
		_ = h.pusher.PushInternalOrder(c.Request.Context(), o.ID, token)
		if refreshed, err := h.svc.Get(c.Request.Context(), o.ID); err == nil && refreshed != nil {
...
func (h *InternalOrderHandler) NextNumber(c *gin.Context) {
	...
	if err != nil {
		c.Error(apperr.Internal("next number", err))
		return
	}
...
func (h *InternalOrderHandler) Print(c *gin.Context) {
	...
	if err := h.svc.MarkPrinted(c.Request.Context(), id); err != nil {
		c.Error(apperr.Internal("mark printed", err))
		return
	}
	o, err := h.svc.Get(c.Request.Context(), id)
	if err != nil {
...
func (h *InternalOrderHandler) Send(c *gin.Context) {
	...
	if err := h.svc.MarkSent(c.Request.Context(), id); err != nil {
		c.Error(apperr.Internal("mark sent", err))
		return
	}
	o, err := h.svc.Get(c.Request.Context(), id)
	if err != nil {
