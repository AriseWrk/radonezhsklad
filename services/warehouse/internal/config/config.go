...
type Config struct {
	...
	// Путь к папке scripts/ (для запуска PS-скриптов синхронизации).
	// По умолчанию "../../scripts" — при запуске из services/warehouse даёт корень/scripts.
	ScriptsDir string
}
...
func Load() *Config {
	return &Config{
		...
		ScriptsDir: shcfg.GetString("SCRIPTS_DIR", "../../scripts"),
	}
}
