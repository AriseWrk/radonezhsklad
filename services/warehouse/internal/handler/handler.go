...
type Handler struct {
	...
	pusher        *mspush.Pusher
}

func New(svc *service.Service, pc *product.Client, pusher *mspush.Pusher) *Handler {
	return &Handler{svc: svc, productClient: pc, pusher: pusher}
}
...
func (h *Handler) CreateDocument(c *gin.Context) {
	...
	for _, it := range req.Items {
		...
		if err != nil {
			c.Error(apperr.BadRequest("invalid product_id"))
			return
		}
		if it.Quantity <= 0 {
			c.Error(apperr.BadRequest("quantity must be > 0"))
			return
		}
...
func (h *Handler) PostDocument(c *gin.Context) {
	...
	// push в МС (после локального коммита); ошибка уже записана в d.ms_sync_error
	if h.pusher != nil && h.pusher.Enabled() {
		token := strings.TrimPrefix(c.GetHeader("Authorization"), "Bearer ")
		_ = h.pusher.PushDocument(c.Request.Context(), d.ID, token)
		if refreshed, err := h.svc.GetDocument(c.Request.Context(), d.ID); err == nil && refreshed != nil {
			d = refreshed
		}
	}
	httpx.OK(c, d)
