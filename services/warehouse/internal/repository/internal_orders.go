...
SELECT o.id, o.number, o.doc_date, o.status, o.organization_id, o.warehouse_id, COALESCE(w.name, '') AS warehouse_name,
       ...
       o.ms_synced_at, o.ms_sync_error,
...
func scanIntOrder(row pgx.Row) (*models.InternalOrder, error) {
	...
	err := row.Scan(&o.ID, &o.Number, &o.DocDate, &o.Status, &o.OrganizationID, &o.WarehouseID, &o.WarehouseName,
		...
		&o.PostedAt, &o.CancelledAt, &o.CreatedBy, &o.CreatedAt, &o.UpdatedAt, &o.ExternalID,
		&o.MSSyncedAt, &o.MSSyncError, &o.ItemsCount)
...
FROM internal_order_items WHERE order_id = $1 ORDER BY created_at`, id)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&it.ID, &it.OrderID, &it.ProductID, &it.Quantity,
			&it.Price, &it.VatRate, &it.Sum, &it.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, it)
	}
...
func (r *InternalOrderRepo) List(ctx context.Context, f InternalOrderFilters) ([]models.InternalOrder, error) {
	...
	i := 1
	if f.Status != nil {
		q += ` AND o.status = $` + itoa(i)
		args = append(args, *f.Status)
		i++
	}
	if f.WarehouseID != nil {
		q += ` AND o.warehouse_id = $` + itoa(i)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&o.ID, &o.Number, &o.DocDate, &o.Status, &o.OrganizationID, &o.WarehouseID, &o.WarehouseName,
			...
			&o.VatEnabled, &o.VatIncluded,
			&o.PostedAt, &o.CancelledAt, &o.CreatedBy, &o.CreatedAt, &o.UpdatedAt, &o.ExternalID,
			&o.MSSyncedAt, &o.MSSyncError, &o.ItemsCount); err != nil {
			return nil, err
		}
		out = append(out, *o)
	}
...
WHERE id=$1`,
		...
		if _, err := tx.Exec(ctx, `DELETE FROM internal_order_items WHERE order_id=$1`, id); err != nil {
			return err
		}
		for _, it := range in.Items {
			sum := it.Quantity * it.Price
...
INSERT INTO internal_order_items (order_id, product_id, quantity, price, vat_rate, sum)
VALUES ($1,$2,$3,$4,$5,$6)`,
				id, it.ProductID, it.Quantity, it.Price, it.VatRate, sum); err != nil {
				return err
			}
		}
...
func (r *InternalOrderRepo) UpdateStatus(ctx context.Context, id uuid.UUID, status string, posted, cancelled bool) error {
	...
	switch {
	case posted:
		q = `UPDATE internal_orders SET status=$2, posted_at=NOW() WHERE id=$1`
	case cancelled:
		q = `UPDATE internal_orders SET status=$2, cancelled_at=NOW() WHERE id=$1`
	default:
		q = `UPDATE internal_orders SET status=$2 WHERE id=$1`
	}
