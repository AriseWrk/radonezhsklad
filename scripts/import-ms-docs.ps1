...
Write-Host "=== Import MS /entity/$Type -> documents.type=$DocType (limit=$Limit) ==="
...
while ($true) {
    ...
    $page = MsApi-Get -Path "/entity/$Type" -Query $q
    ...
    foreach ($d in $page.rows) {
        ...
        $storeU = Get-HrefUuid $d.sourceStore 'store'
        if (-not $storeU) { $storeU = Get-HrefUuid $d.store 'store' }
        $tgtU   = Get-HrefUuid $d.targetStore 'store'
        $agentU = Get-HrefUuid $d.agent 'counterparty'
        $orgU   = Get-HrefUuid $d.organization 'organization'

        $moment = $null
        if ($d.moment) { $moment = ($d.moment -replace '\.\d+$','') + '+03' }
        $incDate = $null
        if ($d.incomingDate) { $incDate = ($d.incomingDate -split ' ')[0] }
        $totalR = 0
        ...
        if ($expected -gt $positions.Count) {
            $extraPosCalls++
            $positions = MsApi-GetAll -Path "/entity/$Type/$($d.id)/positions" -PageSize 1000
        }
...
$tpl = Read-Utf8File -Path (Join-Path $root 'scripts\sql\upsert_documents.sql')
$sql = $tpl
$sql = $sql.Replace('{{DOCS_PATH}}',  '/tmp/ms_docs.tsv')
$sql = $sql.Replace('{{ITEMS_PATH}}', '/tmp/ms_items.tsv')
$sql = $sql.Replace('{{DOC_TYPE}}',   $DocType)
...
if ($KeepTsv) {
    Write-Host "TSVs kept at: $tsvDir"
} else {
